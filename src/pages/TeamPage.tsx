...
const LEADERSHIP = [
  {
    ...
    expertise: ['Strategic Partnerships', 'Supply Chain Management', 'Business Development', 'Market Expansion'],
    initials: 'ZE',
    color: 'bg-primary',
  },
  {
    ...
    expertise: ['Operations Management', 'Logistics & Distribution', 'Quality Control', 'Process Optimization'],
    initials: 'NA',
    color: 'bg-accent',
  },
  {
    ...
    expertise: ['B2B Sales Strategy', 'Account Management', 'Market Analysis', 'Client Relations'],
    initials: 'MO',
    color: 'bg-blue-500',
  },
];
...
const TEAM = [
  {
    ...
    expertise: ['Global Sourcing', 'Supplier Relations', 'OEM Authentication', 'Cost Negotiation'],
    initials: 'AA',
    color: 'bg-green-600',
  },
  {
    ...
    expertise: ['OEM Cross-Referencing', 'European Vehicles', 'Japanese Imports', 'Customer Support'],
    initials: 'SC',
    color: 'bg-purple-600',
  },
  {
    ...
    expertise: ['Fleet Management', 'Route Optimization', 'Last-Mile Delivery', 'Customs & Import'],
    initials: 'JO',
    color: 'bg-orange-600',
  },
  {
    ...
    expertise: ['ISO Compliance', 'Counterfeit Detection', 'Supplier Auditing', 'Quality Standards'],
    initials: 'FA',
    color: 'bg-teal-600',
  },
  {
    ...
    expertise: ['Fleet Accounts', 'Wholesale Contracts', 'Credit Management', 'CRM Systems'],
    initials: 'DK',
    color: 'bg-red-700',
  },
  {
    ...
    expertise: ['ERP Systems', 'B2B Portal', 'Data Analytics', 'Digital Processes'],
    initials: 'PS',
    color: 'bg-indigo-600',
  },
  {
    ...
    expertise: ['Inventory Management', 'Team Leadership', 'Safety Compliance', 'Stock Accuracy'],
    initials: 'KH',
    color: 'bg-amber-600',
  },
  {
    ...
    expertise: ['Customer Relations', 'Complaint Resolution', 'After-Sales Support', 'Team Training'],
    initials: 'LT',
    color: 'bg-pink-600',
  },
];

const DEPTS = ['All', 'Leadership', 'Procurement', 'Technical', 'Logistics', 'Quality', 'Sales', 'Technology', 'Operations', 'Customer Service'];

function MemberCard({ member, delay = 0 }: { member: typeof TEAM[0]; delay?: number }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="relative h-40 bg-secondary flex items-center justify-center overflow-hidden">
        <div className={`w-20 h-20 rounded-full ${member.color} flex items-center justify-center text-white text-2xl font-display font-bold shadow-lg group-hover:scale-110 transition-transform duration-300`}>
          {member.initials}
        </div>
        ...
        {/* Decorative gradient */}
        <div className={`absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-500 ${member.color}`} />
      </div>
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {member.expertise.map((e) => (
            <span key={e} className="px-2 py-0.5 rounded text-[10px] font-semibold bg-secondary border border-border text-muted-foreground uppercase tracking-wider">
              {e}
            </span>
          ))}
        </div>
    ...
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="max-w-3xl">
            ...
            <p className="text-muted-foreground text-lg leading-relaxed">
              Our team of automotive specialists, logistics experts, and customer champions work together every day to ensure you receive the right part, at the right price, at the right time.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-4 mb-12">
            ...
            <h2 className="text-2xl font-display font-bold text-foreground uppercase tracking-widest whitespace-nowrap">Leadership</h2>
            <div className="h-px flex-1 bg-border" />
          </div>
      ...
      <section className="py-20 bg-secondary border-t border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-4 mb-12">
            <div className="h-px flex-1 bg-border" />
            <h2 className="text-2xl font-display font-bold text-foreground uppercase tracking-widest whitespace-nowrap">Our Specialists</h2>
            <div className="h-px flex-1 bg-border" />
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-accent/30 bg-accent/10 mb-6">
                <span className="text-accent text-xs font-bold uppercase tracking-widest">Why Work With Us</span>
              </div>
              <h2 className="text-3xl md:text-5xl font-display font-bold text-foreground mb-6">
                A CULTURE OF <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">EXCELLENCE</span>
              </h2>
              <p className="text-muted-foreground leading-relaxed mb-6">
                At ZEB Trading, we believe great people build great companies. Our team is our greatest asset, and we invest in their growth, well-being, and professional development at every step.
              </p>
            ...
            >
              {[
                { val: '11', label: 'Years Avg Experience', color: 'bg-primary/10 border-primary/20' },
                { val: '12+', label: 'Nationalities on Team', color: 'bg-accent/10 border-accent/20' },
                { val: '97%', label: 'Staff Retention Rate', color: 'bg-green-500/10 border-green-500/20' },
                { val: '25+', label: 'Team Members', color: 'bg-blue-500/10 border-blue-500/20' },
              ].map((stat) => (
                <div key={stat.label} className={`${stat.color} border rounded-2xl p-6 text-center`}>
                  <p className="text-4xl font-display font-bold text-foreground mb-2">{stat.val}</p>
      ...
      <section className="py-16 bg-primary">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="text-2xl md:text-4xl font-display font-bold text-white mb-4">HAVE A QUESTION FOR OUR TEAM?</h2>
          <p className="text-white/80 mb-8">Our specialists are available 24/7 to assist with part sourcing, technical queries, and wholesale enquiries.</p>
          <div className="flex flex-wrap justify-center gap-4">
            <Link href="/contact" className="flex items-center gap-2 px-8 py-4 rounded-sm font-bold uppercase tracking-wider text-sm bg-white text-primary hover:bg-gray-100 transition-all">
              Contact Us <ArrowRight className="w-4 h-4" />
            </Link>
            <a href="tel:+97141234567" className="flex items-center gap-2 px-8 py-4 rounded-sm font-bold uppercase tracking-wider text-sm border-2 border-white/50 text-white hover:bg-white/10 transition-all">
